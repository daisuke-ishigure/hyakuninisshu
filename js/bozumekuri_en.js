/**
 * Hyakunin Isshu Memory Match Game (English)
 */
...
function getCommonHeaderHTML(showTitle = true) {
    ...
    <nav>
        <ol class="breadcrumb">
            ...
            <li>Hyakunin Isshu Memory Match</li>
        </ol>
    </nav>
    <hr>`;
    ...
    if (showTitle) {
        html += `
        <div class="title">
            <h1>Hyakunin Isshu Memory Match</h1>
        </div>`;
    }
    return html;
}
...
function createSelectionScreen() {
    ...
    <main class="main_about-main">
        <div class="section">
            ...
            <ul id="karuta_ul">
                <li>Match the "upper verse" and "lower verse" of the same poem.</li>
                <li>Practice with just the 20 poems from a single color of the Five-Color Hyakunin Isshu set.</li>
                <li>The chosen number of pairs is drawn at random.</li>
            </ul>
            <div class="quiz-controls-wrapper">
                <div class="quiz-controls">
                    <p>① Choose a card color.</p>
                ...
                <div class="quiz-controls">
                    <p>② Choose the number of pairs.</p>
                    <select id="quiz-limit-select">
                        <option value="2">2 pairs (4 cards)</option>
                        <option value="4" selected>4 pairs (8 cards)</option>
    ...
    const startBtn = document.getElementById('start-button_en');
    if (startBtn) {
...
function startMemoryGame(quizType) {
    ...
    if (!sourceSongs || sourceSongs.length === 0) return alert('No poems found.');
    ...
    container.innerHTML = `
        <main class="main_about-main">
            <div class="section">
                ${getCommonHeaderHTML(true)}
                <h3 class="question-number">Matched pairs: <span id="match-count">0</span> / ${quizQuestionsLimit}</h3>
...
function showResults() {
    ...
    // X共有用リンク作成
    const tweetText = encodeURIComponent(`I cleared Hyakunin Isshu Memory Match [${quizQuestionsLimit} pairs] in [${clearTime}s]!`);
    ...
    container.innerHTML = `
        <main class="main_about-main">
            <div class="section" style="text-align: center;">
                ...
                <div class="title">
                <h2>You matched every pair!</h2>
                </div>
                <p style="font-size: 1rem; margin-bottom: 20px; text-align:center;">
                    Clear time: <strong>${clearTime}s</strong>
                </p>
                <div style="margin-bottom: 25px;">
                    <a href="${twitterLink}" target="_blank" rel="noopener noreferrer">
                        <div id="share_button_en" style="display: block;">Share on X</div>
                    </a>
                </div>
                <p style="font-size: 0.9rem; color: #666; text-align:center;">Returning to the start screen in 5 seconds</p>
            </div>
        </main>`;
