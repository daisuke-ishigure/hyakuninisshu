/* ============================================================
   下の句だんご - shimonoku-dango.js
   上の句をお題にして、下の句（二句）を順番に串へ刺していくゲーム。
   kaminoku-dango.js のゲームエンジンを流用し、データは五句並べ
...
async function init() {
  ...
  // 五句並べの5句データから、下の句（4〜5句目＝刺していく札）と
  // 上の句（1〜3句目＝お題として表示）を組み立てる
  allPoems = raw.map(poem => ({
    id: poem.id,
    poet: poem.poet,
    phrases: poem.phrases,
    segments: poem.phrases.slice(3, 5),
    target: poem.phrases[0] + ' ' + poem.phrases[1] + ' ' + poem.phrases[2],
  }));
