...
.karuta_card.correct-pop { animation: karutaCorrectPop 0.5s ease-out forwards; }
...
const startButton       = document.getElementById('start_button_en');
...
const replayButton      = document.getElementById('replay_button_en');
...
function playNextAudio() {
    ...
    if (currentAudioInstance) {
        currentAudioInstance.muted = false;
        currentAudioInstance.currentTime = 0;
        const p = currentAudioInstance.play();
        if (p !== undefined) {
            p.catch(err => {
...
function spawnSparkles(button) {
    ...
    const waves = [
        ...
        { count: 18, distMin:  60, distMax: 110, baseDelay:  55 },
        { count: 13, distMin:  90, distMax: 150, baseDelay: 115 }
    ];
    ...
    waves.forEach(({ count, distMin, distMax, baseDelay }) => {
        for (let i = 0; i < count; i++) {
            ...
            const delay  = baseDelay + Math.random() * 70;
            ...
            const dur    = (0.95 + Math.random() * 0.55).toFixed(2) + 's';
...
function cardClickHandler(song, element) {
    ...
    if (String(song.number) === String(currentSongNumber)) {
        ...
        resultText.textContent = 'Wrong card!';
        mistakeCount++;
...
function finishGame() {
    ...
    const mistakeMsg = mistakeCount === 0 ? "Perfect!" : `Mistakes: ${mistakeCount}`;

    resultText.innerHTML = `
        <div style="font-size:1.4rem; font-weight:bold; margin-bottom:10px;">
            You cleared the game!
        </div>
        Time: ${finalScore.toFixed(2)} sec<br>
        Per card: ${avgTime.toFixed(2)} sec<br>
        ${mistakeMsg}
    ...
    document.dispatchEvent(new CustomEvent('karutaEnGameFinished', {
        detail: { finalScore, totalCards, mistakeCount }
    }));
}
...
function buildKimarijiChartHTML() {
    ...
    const donuts = entries.map(([len, { correct, total }], i) =>
        buildDonutSVG((suffix[Number(len)] || len + '-char') + ' kimariji', correct, total, 300 + i * 120)
    ).join('');
    ...
    <div style="margin:20px auto 8px;max-width:600px;text-align:center;">
        <p style="font-size:0.85rem;font-weight:bold;color:#666;margin-bottom:10px;text-align:center;">
            ── Accuracy by kimariji length (fault-free only) ──
        </p>
...
function setupShareButton(score, cards, mistakes) {
    ...
    shareButton.onclick = () => {
        const text = encodeURIComponent(
            `[Hyakunin Isshu Karuta Game]\nClear time: ${score} sec\nCleared ${cards} cards with ${mistakes} mistake(s)!\n`
        );
