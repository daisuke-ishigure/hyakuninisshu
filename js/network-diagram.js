/* 百人一首 相関図
   データ: network-data.json / スタイル: network-diagram.css */
(async function () {
      ...
      let networkConfig;
      try {
        const res = await fetch('js/network-data.json?20260719-3');
        if (!res.ok) throw new Error('HTTP ' + res.status);
        networkConfig = await res.json();
      } catch (err) {
        console.error('network-data.json の読み込みに失敗:', err);
        const msg = document.createElementNS('http://www.w3.org/2000/svg', 'text');
        msg.setAttribute('x', '20'); msg.setAttribute('y', '40');
        msg.setAttribute('font-size', '14'); msg.setAttribute('fill', '#c0392b');
        msg.textContent = 'データの読み込みに失敗しました（file://直接表示の場合はローカルサーバーをご利用ください）';
        svg.appendChild(msg);
        return;
      }
      ...
      const CHAR_W   = 13;   // 日本語1文字あたりの概算幅(px)
      const PAD_X    = 14;   // 左右パディング
      ...
      const totalH  = lastEra.y + (lastEra.maxRows - 1) * ROW_GAP + NODE_H + 40;
      ...
      const relationshipStyles = {
        'parent-child':    { color: '#e74c3c', legend: '親子',          dash: null  },
        'blother':         { color: '#e67e22', legend: '兄弟',          dash: null  },
        'sofu-mago':       { color: '#8e44ad', legend: '祖父と孫',      dash: null  },
        'sosofu-himago':   { color: '#6c5ce7', legend: '曽祖父と曽孫', dash: null  },
        'teacher-student': { color: '#00897b', legend: '師弟・歌聖',    dash: '7,4' },
        'love':            { color: '#ff3d77', legend: '恋愛',          dash: null  },
        'influence':       { color: '#2e9e5b', legend: '影響',          dash: '4,4' },
        'friend':          { color: '#1273b5', legend: '交流',          dash: '4,4' },
        'rival':           { color: '#5f6a72', legend: 'ライバル',      dash: '2,4' },
        'cousin':          { color: '#c48a2e', legend: 'いとこ',        dash: '7,4' },
        'ancestor':        { color: '#795548', legend: '先祖',          dash: '10,4' },
        'dream':           { color: '#7b68ee', legend: '夢枕',          dash: '1,4' },
        'worship':         { color: '#b8860b', legend: '崇拝',          dash: '3,2' },
        'uncle-nephew':    { color: '#d17b3a', legend: '叔父と甥',      dash: '6,3' },
        'depicted':        { color: '#546e7a', legend: '説話',          dash: '5,2,1,2' },
        'rokkasen':        { color: '#00acc1', legend: '六歌仙',        dash: '3,3'  },
        'compiler':        { color: '#7e57c2', legend: '撰者仲間',      dash: '2,6'  },
        'honkadori':       { color: '#00838f', legend: '本歌取り',      dash: '4,1'  },
        'in-law-brother':  { color: '#7cb342', legend: '義兄弟',        dash: '6,2,2,2' },
        'lord-vassal':     { color: '#ad1457', legend: '君臣',          dash: '8,2'  },
        'pilgrimage':      { color: '#558b2f', legend: '聖地巡礼',      dash: '5,3,1,3', arrow: true },
        'separation':      { color: '#943126', legend: '仲を引き裂く',  dash: '6,2,1,2', arrow: true }
      };
      const FALLBACK_STYLE = { color: '#999', legend: 'その他', dash: null };
      ...
      function relEndpoints(rel) {
        ...
        const y1 = getPoetY(fp) + NODE_H / 2;
        const x2 = tp.x + nodeWidth(tp) / 2;
        const y2 = getPoetY(tp) + NODE_H / 2;
      ...
      function render() {
        ...
        networkConfig.relationships.forEach((rel, relIndex) => {
          ...
          if (rel.bend != null) {
            ...
            poetStates.forEach(p => {
              ...
              if (dist < threshold) {
                offset = Math.max(offset, threshold - dist + 30);
              }
            });
            if (offset < 22) offset = 22; // 最低限の曲がりを持たせる
          }
          ...
          if (showArrow) {
            ...
            const hw = nodeWidth(tp) / 2, hh = NODE_H / 2;
            const margin = 6;
          ...
          // 曲線上の別の位置を自動的に探して回避する。
          const labelCandidates = [];
          if (rel.labelT != null) labelCandidates.push(rel.labelT);
          [0.5, 0.35, 0.65, 0.25, 0.75, 0.15, 0.85].forEach(t => {
            if (!labelCandidates.includes(t)) labelCandidates.push(t);
          });

          const labelHalfW = rel.label.length * 6 + 8;
          const labelHalfH = 10;
          let lx, ly, bestCollisions = Infinity;
          for (const t of labelCandidates) {
            const mt = 1 - t;
            const clx = mt * mt * x1 + 2 * mt * t * cx + t * t * x2;
            const cly = mt * mt * y1 + 2 * mt * t * cy + t * t * y2;
            ...
            poetStates.forEach(p => {
              ...
              const py1 = getPoetY(p) - 8, py2 = getPoetY(p) + NODE_H - 8;
        ...
        poetStates.forEach(poet => {
          ...
          text.setAttribute('y', ny + NODE_H / 2);
      ...
      function handlePointerMove(e) {
        ...
        if (focusedId !== null) {
          ...
          newDY = Math.max(-baseY, Math.min(totalH - NODE_H - baseY, newDY));
      ...
      if (toggleBtn) {
        toggleBtn.addEventListener('click', () => {
          ...
          toggleBtn.textContent = editMode ? '🔧 調整モード ON' : '🔧 調整モード OFF';
      ...
      if (copyBtn) {
        copyBtn.addEventListener('click', () => {
          ...
          const done = () => {
            ...
            setTimeout(() => { copyBtn.textContent = '📋 座標をコピー'; }, 1800);
          };
      ...
      // 調整結果を network-data.json に直接上書き保存する（Chrome/Edge等のFile System Access API対応ブラウザのみ）。
      // 初回はファイル選択ダイアログで js/network-data.json を選んでもらう必要がある
      ...
      async function saveJsonToFile() {
          ...
          try {
            if (!savedFileHandle) {
              savedFileHandle = await window.showSaveFilePicker({
                suggestedName: 'network-data.json',
        ...
        // フォールバック: ダウンロードとして保存し、js/network-data.json に手動で上書きしてもらう
      ...
      if (saveBtn) {
        saveBtn.addEventListener('click', async () => {
          ...
          if (ok) {
            ...
            setTimeout(() => { saveBtn.textContent = '💾 ファイルに保存'; }, 1800);
          }
        });
      }
      ...
      function showPoetTooltip(poet, clientX, clientY) {
        ...
        tipEl.innerHTML =
          ...
          `<div class="tip-hint">タップ・クリックで解説ページへ →</div>`;
